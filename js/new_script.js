...
// تحديث دالة switchTab لتشمل نظام التبويب الثاني
function switchTab(tabId) {
  document.querySelectorAll('.tab-content').forEach(tab => tab.classList.remove('active'));
  document.getElementById(tabId).classList.add('active');
...
// تحديث دالة handleReligiousSubmit لحفظ البيانات
...
// تحديث initColorCodingSystem الرئيسي ليشمل التبويب الثاني
function initCompleteColorCodingSystem() {
    // نظام التبويب الأول
    initColorCodingSystem();
    
    // نظام التبويب الثاني (إذا كان التبويب نشطاً)
    if (document.querySelector('#religious.tab-content.active')) {
        setTimeout(() => {
            initTab2ColorCodingSystem();
        }, 200);
    }
...
// استدعاء النظام الشامل عند التحميل
document.addEventListener('DOMContentLoaded', function() {
    // ... الكود الحالي ...
